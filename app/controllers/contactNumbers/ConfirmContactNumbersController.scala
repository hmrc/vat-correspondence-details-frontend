...
@Singleton
class ConfirmContactNumbersController @Inject()(val authComps: AuthPredicateComponents,
  ...
  def show: Action[AnyContent] = allowAgentPredicate { implicit user =>
    ...
    (prepopulationLandline, prepopulationMobile) match {
      ...
      case _ => Ok(confirmContactNumbersView(numberToShow(prepopulationLandline, validationLandline), numberToShow(prepopulationMobile, validationMobile)))
    }
  }

  def updateContactNumbers: Action[AnyContent] = allowAgentPredicate.async { implicit user =>
